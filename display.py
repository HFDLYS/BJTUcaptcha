...
class DrawingCanvas(QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.setFixedSize(MODEL_INPUT_WIDTH * 2 + 20, MODEL_INPUT_HEIGHT * 2 + 20) # Slightly larger to see borders
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            # Map view point to scene point, then to item point (which is image coords)
...
class CaptchaInferenceApp(QWidget):
    def __init__(self):
        ...
        self.setGeometry(100, 100, 650, 550) # Increased size a bit
    ...
    def init_ui(self):
        ...
        canvas_container_layout.addWidget(self.drawing_canvas)
        canvas_container_layout.addStretch()
        ...
        self.lbl_pen_color_preview = QLabel() # To show current pen color
        self.lbl_pen_color_preview.setFixedSize(20, 20)
        self.update_pen_color_preview(self.drawing_canvas.pen_color) # Initialize preview

        self.btn_bg_color = QPushButton("Background Color")
        self.btn_bg_color.clicked.connect(self.choose_background_color)
        self.lbl_bg_color_preview = QLabel() # To show current background color
    ...
    def load_image_dialog(self):
        ...
        if fname:
            try:
                ...
                self.lbl_image_display_file.setPixmap(pixmap.scaled(
                    self.lbl_image_display_file.width() - 10 , # Give some padding
    ...
    def run_inference(self):
        ...
        if active_tab_index == 0:
            if self.current_pil_image:
                image_to_infer = self.current_pil_image.copy() # Use a copy
            else:
        ...
        try:
            ...
            preds_tensor = ort_outs[0]
            preds_idx = np.argmax(preds_tensor[:, 0, :], axis=1)
